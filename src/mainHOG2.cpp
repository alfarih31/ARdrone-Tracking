...
void tracking_target(ARDrone &ardrone, Mat full_image, Mat target, Rect2d target_pos)
{
    ...
    while(1)
    {
        ...
        if(!ff_image.empty() && blurry >= blur_t)
        {
            ...
            bool show = SURF_matching(detected_target, target);
            if(show && ok)
            {
                n++;
...
void filter_target(Mat &src, Rect &_r)
{
    ...
    Mat t_image, c_image, g_image;
    vector<vector<Point> > contours;
    vector<Vec4i> hierarchy;
    cvtColor(src, g_image, COLOR_BGR2GRAY);
    GaussianBlur(g_image, g_image, Size (3, 3), 0, 0);
    threshold(g_image, t_image, 127, 255, THRESH_OTSU);
    Canny(t_image, c_image, 127, 255, 3);
    findContours(c_image, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point (0, 0));
    ...
    for(int i = 0; i < contours.size(); i++)
    {
        ...
        if (area > maxArea) maxArea = area, maxCon = contours[i];
    }

    double ymin = src.rows, xmax = 0, xmin = src.cols;
    Rect2d roi;
    ...
    roi.height = (src.rows/2 - ymin)+1;


    for (int i = 0; i < maxCon.size(); i++)
    {
        if(maxCon[i].y <= roi.height)
        {
            double x = maxCon[i].x;
            if(x > xmax) xmax = x;
            if(x < xmin) xmin = x;
...
/*    double maxD = 0;
    ...
    for(int i = 0; i < maxCon.size(); i++)
    {
        double y = maxCon[i].y;
        double x = maxCon[i].x;
        for(int j = 0; j < maxCon.size(); j++)
        {
            if(maxCon[j].y == y)
            {
                double dist = maxCon[j].x - x;
                if(dist > maxD) maxD = dist, n2 = j, n1 = i;
            }
        }
    }
*/
        ...
        if(found.size() != 0)
        {
            for (int i = 0; i <= found.size(); i++)
            {
                ...
                if (ratio >= 1.61 && ratio <= 1.99) // Average Ratio of human body
                {
                    try 
                    {
                        crop_image(flat_image, target, r);
